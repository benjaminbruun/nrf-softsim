...
int nrf_softsim_provision(uint8_t *profile_r, size_t len)
{
	...
	int rc = ss_profile_from_string((uint16_t)len, (const char *)profile_r, &profile);

	if (rc != 0) {
		LOG_ERR("SoftSIM failed to decode profile (err %d)", rc);
		return -EINVAL;
	}

	/* Import keys into PSA/KMU hardware */
